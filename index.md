...
| `src/config/ips-v1.ts` | `IPS_V1_TEXT` — PRD §3.1–3.10 verbatim as a template literal (incl. the owner's 2026-09-19 ≤10% cash-ceiling amendment to §3.3). A TS module, not a `.md` read at import time: the Next.js bundle can neither run `readFileSync(new URL(…, import.meta.url))` nor ship the file to Vercel. `tests/domain/ips-verbatim.test.ts` holds it to the PRD |
...
| `src/domain/ips.ts` | `IPS_V1_TEXT`, `installIps(db, opts?)`, `currentIps(db)`, `ipsClause(text, clause)`, `renderIps(text, clause?)` — PRD §3.1–3.10 verbatim in `src/config/ips-v1.ts`, versioned storage in `ips_versions`, idempotent install, clause extraction for FR-10 citations. `currentIps` normalises `effective_at` to an ISO string — postgres-js returns a `Date`, PGlite a string |
...
| `src/jobs/backfill-prices.ts` | CLI — `pnpm backfill:prices --days=260 --end=YYYY-MM-DD`. Drives `downloadBhavcopy`/`downloadIndexSeries`/`ingestPrices` over past trading days so the 200DMA has history. Resumable: a day is skipped only when BOTH `prices_eod` and `index_prices_eod` hold it. `as_of` is the session, not the run — **added 2026-09-21** |
| `src/jobs/backfill-navs.ts` | CLI — `pnpm backfill:navs --months=30 --end=YYYY-MM`. Chunks AMFI's historical report by month (it ignores per-scheme filtering). Exports `monthsBack`, `monthBounds`, `amfiDate`. Resumable: a month is held when every tracked MF instrument has a NAV in it — **added 2026-09-21** |
| `src/domain/mf-metadata.ts` | `MfMetadata`, `persistMfMetadata`, `loadMfMetadata`, `aumCroreToPaise`, `expensePctToBps`, `CRORE_PAISE` — expense ratio and AUM from INDmoney's `get_mf_funds_details`. `aum` is RUPEES CRORE (verified against known fund sizes) and `expense_ratio` a percent. `loadMfMetadata` keys results under the instrument id AND its `canonical_id` — **added 2026-09-21** |
| `src/domain/mf-universe.ts` | `SWITCHABLE_CATEGORIES`, `categoriesHeld`, `candidatesIn`, `persistUniverse`, `loadUniverse`, `stampHeldCategories`, `universeInstrumentId` — the `mf_switch` candidate universe from AMFI's own category headings. Candidates are `MFU:<schemeCode>` and must be provably Direct + growth; a HELD fund never is, because AMFI leaves Plan/Option blank for some. Index funds excluded (category says a fund is an index fund, not which index) — **added 2026-09-21** |
| `src/domain/mf-switch.ts` | `evaluateSwitches`, `SWITCH_MARGIN` (10 on a 75-point scale), `MIN_HISTORY_SHARE` (0.9) — each holding against its whole AMFI category. Every series is trimmed to ONE common span: 31 rolling windows against 30 is not a comparison. Both sides must be priced or the holding wins on metadata alone — **added 2026-09-21** |
| `src/sources/mf-peers.ts` | `fetchCategoryPeers`, `fetchAllInCategory`, `resolvePeerToAmfi`, `resolvePeers`, `isoFromIndmoneyDate`, `parseAumCrore`, `INDMONEY_CATEGORY_SLUG` — INDmoney's fund id is NOT an AMFI scheme code and its payload carries no ISIN, so a listing is joined to a scheme by NAV FINGERPRINT (name + NAV to the paisa) against AMFI's file for the peer's own `nav_date` — **added 2026-09-21** |
| `src/jobs/build-mf-universe.ts` | CLI — `pnpm mf:universe --months=30`. Builds the universe, loads month-end NAVs, then `enrichUniverse` prices candidates from `get_mf_by_category` — **added 2026-09-21** |
| `src/domain/mf-ranking.ts` | `rankHeldFunds`, `heldFunds`, `monthEndNavs`, `unevenHistory`, `MAX_ACHIEVABLE_COMPOSITE` (75), `MONTHS_PER_WINDOW` (12) — the only production caller of `rankMfs`. NAVs are downsampled to MONTH ENDS first, or a 12-point window measures twelve DAYS. The series is found through `canonical_id`. Emits no `mf_switch`: a switch needs a candidate universe, which is an owner decision — **added 2026-09-21** |
| `src/domain/report-runs.ts` | `alreadyReportedFor`, `recordReportRun`, `loadReportRuns`, `ReportRunRecord` — the delivered weekly report and what it said (narration, engine bullets, sent text) in the `audit_log` row that already recorded the run. **Lives in `domain` because the web reads it**: importing `jobs/report.ts` pulls in `runMigrations` and `next build` cannot resolve its dynamic `../../migrations` import — **added 2026-09-21** |
...
| File | Exports / role |
|---|---|
| `web/next.config.ts` | two things: parses repo-root `.env` itself (`KEY=value`, skips `#`, sets env only if unset) — **Next has no `envDir`**; `resolve.extensionAlias {'.js':['.ts','.tsx','.js']}` for src's ESM `.js` specifiers |
...
| `src/sources/amfi.ts` | `parseNavText`, `parseNavHistory`, `downloadDailyNav`, `downloadNavHistory(from,to)`, `ingestNavs` — AMFI daily + historical NAV → `navs` (`nav_micros`, BIGINT). History is SEMICOLON-delimited with eight columns and is parsed by HEADER NAME, because the daily and historical files order their columns differently. `downloadNavHistory` takes a date range, not a scheme: AMFI ignores `&sc=` and serves the whole ~15MB report regardless — **history fixed 2026-09-21** |
...
| `src/sources/screener-screen.ts` | screener.in **screen HTML** → `fundamentals` — `parseScreenHtml(html)` (data-row-company-id rows; header tooltips → canonical keys; `<span>` units stripped; per-column normalization), `fetchScreen(url)` (paginated via `URL.searchParams.set('page', n)` **so query-bearing `/screen/raw/?query=...` URLs page correctly** — the old `base + '?page=N'` stripped queries on page 2; 25/page, stops on short page, default page budget 100 as a safety valve (a 10-page default silently truncated the owner's 410-record/17-page sentinel screen 3963033)), `slugToInstrumentId(db, slug)`, `importScreenRows(db, rows, {asOf?, screenUrl})` → **`{ uploadedId, inserted, createdInstruments, warnings }`** — idempotent per `(as_of, filename)`, deletes prior batch on re-import because `screener_uploads` is append-only; **cohort promotion 2026-09-15**: each row whose company is not yet in `instruments` is created (`NSE:<slug>`, kind EQUITY, `metadata {"source":"screener-cohort"}`, ISIN NULL for `backfill:isin`) via internal `ensureScreenInstrument` (not exported), so the universe widens on every import; degenerate `/company/id/<n>/` slugs (len<3) refused. Built because the CSV export is paywalled; a plain screen URL is public. Requires a screen whose columns carry the §6 gate inputs — **implemented 2026-09-13, promotion 2026-09-15** |
...
| `src/domain/rails.ts` | `RAIL_CONSTRAINTS`, `DEFAULT_OWNER_RAILS`, `RailViolation`, `checkRails(db, rec, opts?)` (per-order), `checkPortfolioRails(db, positions, totalAssets)`, `getFreezeState`/`setFreeze`/`checkFreeze`, `getBreakerState`/`recordFalsification`/`resetBreaker` — Phase 2 Task 2. **Only `cash_ceiling_pct` is read from `settings_rails`** (`checkCashCeiling`, added 2026-09-19 for PRD §3.3's ≤10% ceiling; CASH class only, inclusive, `DEFAULT_OWNER_RAILS` as fallback, and the funded B3 balance from `bucket_flows` subtracted first — read directly, never through `buckets.ts`). Every other rail is a hard-coded literal (`100_00_000n` order cap, `50_00_000n` monthly tactical, 12-month hold, >15%/≥20% drawdown gates, 3-strike breaker) or a `CAPS` constant in `allocation.ts`, despite PRD §11 saying all rails live in the table. `RAIL_CONSTRAINTS` is exported and imported by nobody. **Known defect:** `checkPortfolioRails` compares the whole equity book's market value to the ₹50k/month flow budget, so `TACTICAL_BUDGET_EXCEEDED` fires permanently |
...
| `migrations/0018_cash_ceiling_rail.sql` | **applied to Supabase 2026-09-19.** Owner amendment: inserts `cash_ceiling_pct = 10` and deletes the legacy dotted-and-fraction `cash.ceiling = 0.2` key, so `settings_rails` carries one key in one unit |
