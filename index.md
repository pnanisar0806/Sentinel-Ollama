...
| `src/domain/networth.ts` | `InstrumentKind` (mirrors the schema check constraint, `'LOAN'` included), `AssetClass`, `Position` (carries `sector` and `currency`), `NetWorth`, `classify` (throws on LOAN — a liability must never be summed into assets), `loadPositions(db, businessDate?)` (latest snapshot per source, merged with the C-A reconciliation: a seed row is dropped only when a live source covers its `(canonical_id ?? instrument_id, account)` key), `netWorth`, `outstandingLiabilities(db, asOfMonth)` (lateral join; falls back to `loans.outstanding_paise` for a month before the schedule starts) |
...
| `src/notify/telegram-bot.ts` | `TelegramBot` — long-polling command bot (`/sync`, `/status`, `/help`), owner-locked; builds sync sources from exported `indmoneySource` + optional Kite |
...
387/387 across 49 files (2026-08-24).
