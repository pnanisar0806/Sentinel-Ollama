...
| `src/config/ips-v1.ts` | `IPS_V1_TEXT` — PRD §3.1–3.10 verbatim as a template literal. A TS module, not a `.md` read at import time: the Next.js bundle can neither run `readFileSync(new URL(…, import.meta.url))` nor ship the file to Vercel. `tests/domain/ips-verbatim.test.ts` holds it to the PRD |
