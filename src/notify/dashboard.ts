...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .brand h1 { font-size: 2.25rem; font-weight: 800; background: linear-gradient(135deg, #fff 30%, #c7d2fe 70%, #f472b6 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: -0.03em; }
    .brand p { color: var(--text-dim); font-size: 0.95rem; margin-top: 0.35rem; }
    .header-meta { display: flex; align-items: center; gap: 1rem; }
    .last-updated { background: rgba(255,255,255,0.04); padding: 0.5rem 1.25rem; border-radius: 9999px; font-size: 0.85rem; color: var(--text-dim); border: 1px solid var(--border); display: flex; align-items: center; gap: 0.5rem; }
    .last-updated::before { content: ''; width: 8px; height: 8px; background: var(--green); border-radius: 50%; box-shadow: 0 0 8px var(--green); }
    .dashboard-link { display: inline-flex; align-items: center; gap: 0.5rem; background: var(--indigo); color: white; padding: 0.5rem 1rem; border-radius: 0.5rem; font-weight: 600; text-decoration: none; font-size: 0.875rem; }
    ...
    .stat-card { background: var(--card); backdrop-filter: blur(16px); border: 1px solid var(--border); border-radius: 1.25rem; padding: 1.5rem; position: relative; overflow: hidden; transition: all 0.3s; box-shadow: 0 4px 30px rgba(0,0,0,0.2); }
    ...
    .stat-label { font-size: 0.825rem; font-weight: 700; color: var(--text-dim); text-transform: uppercase; letter-spacing: 0.075em; }
    .stat-val { font-size: 1.85rem; font-weight: 800; margin-top: 0.5rem; letter-spacing: -0.02em; }
    .stat-sub { font-size: 0.85rem; margin-top: 0.5rem; display: flex; align-items: center; gap: 0.35rem; color: var(--text-muted); }
    ...
    .card { background: var(--card); backdrop-filter: blur(16px); border: 1px solid var(--border); border-radius: 1.25rem; padding: 1.75rem; box-shadow: 0 4px 30px rgba(0,0,0,0.2); }
    .charts-grid { display: grid; grid-template-columns: 1.1fr 1fr; gap: 2rem; }
    @media (max-width: 1024px) { .charts-grid { grid-template-columns: 1fr; } }
    .chart-container { position: relative; width: 100%; height: 330px; }

    .table-wrapper { overflow-x: auto; border-radius: 1rem; border: 1px solid var(--border); background: rgba(0,0,0,0.15); }
    table { width: 100%; border-collapse: collapse; text-align: left; font-size: 0.9rem; }
    th { background: rgba(255,255,255,0.02); color: var(--text-dim); font-weight: 700; padding: 1rem 1.25rem; border-bottom: 1px solid var(--border); text-transform: uppercase; font-size: 0.725rem; letter-spacing: 0.075em; }
    td { padding: 1rem 1.25rem; border-bottom: 1px solid var(--border); color: var(--text); vertical-align: middle; }
    tr:hover td { background: rgba(255,255,255,0.015); }
    .no-data { text-align: center; color: var(--text-muted); padding: 2rem !important; }
    .num { font-family: 'Outfit', monospace; font-size: 0.925rem; font-weight: 600; }
    ...
    .allocation-table td { padding: 0.75rem 1rem; }

    .bucket-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem; }
    .bucket-card { background: var(--bg-elevated); border: 1px solid var(--border); border-radius: 1rem; padding: 1.5rem; }
    .bucket-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; }
    .bucket-header h4 { font-size: 1rem; font-weight: 700; }
    .bucket-status { font-size: 0.7rem; font-weight: 700; padding: 0.25rem 0.5rem; border-radius: 999px; background: var(--yellow); color: var(--bg); }
    ...
    .progress-fill { height: 100%; background: linear-gradient(90deg, var(--indigo), var(--purple)); border-radius: 4px; transition: width 0.5s; }
    .progress-labels { display: flex; justify-content: space-between; font-size: 0.8rem; color: var(--text-dim); margin-top: 0.5rem; }
    .bucket-mandate { font-size: 0.8rem; color: var(--text-muted); }

    .milestone-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 1.5rem; }
    .milestone-card { background: var(--bg-elevated); border: 1px solid var(--border); border-radius: 1rem; padding: 1.5rem; }
    .milestone-card.open { border-left: 4px solid var(--red); }
    .milestone-card h4 { font-size: 1rem; font-weight: 700; margin-bottom: 0.5rem; }
    .milestone-card p { color: var(--text-dim); font-size: 0.875rem; margin-bottom: 0.75rem; }
    .days-outstanding { font-size: 0.8rem; color: var(--red); font-weight: 600; }

    .vest-card { background: var(--bg-elevated); border: 1px solid var(--border); border-radius: 1rem; padding: 1.5rem; max-width: 300px; }
    .vest-card h4 { font-size: 1rem; font-weight: 700; margin-bottom: 0.5rem; }
    .vest-date { color: var(--text-dim); font-size: 0.875rem; margin-bottom: 0.25rem; }
    .vest-amount { font-size: 1.5rem; font-weight: 800; color: var(--green); }

    .staleness-table { width: 100%; }
    .staleness-row { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 1rem; padding: 1rem; background: var(--bg-elevated); border: 1px solid var(--border); border-radius: 0.75rem; margin-bottom: 0.75rem; align-items: center; }
    .staleness-row.unimplemented { opacity: 0.5; }
    .state-badge { padding: 0.25rem 0.75rem; border-radius: 999px; font-size: 0.7rem; font-weight: 700; text-transform: uppercase; }
    ...
    @media (max-width: 640px) {
      body { padding: 1rem; }
      .staleness-row { grid-template-columns: 1fr; gap: 0.5rem; }
    }
  </style>
</head>
<body>
