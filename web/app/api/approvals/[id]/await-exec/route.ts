...
export async function POST(
  ...
  if (intent.status !== 'ACKNOWLEDGED') {
    return NextResponse.json({ error: `Cannot await execution from status: ${intent.status}` }, { status: 400 });
  }
  
  const newStatus = 'AWAITING_MANUAL_EXECUTION';
  ...
  await d.query(
    `insert into audit_log (entity, entity_id, action, actor, payload)
     values ('order_intent', $1, 'AWAIT_MANUAL_EXECUTION', 'owner', $2::jsonb)`,
    [id, JSON.stringify({})],
  );
